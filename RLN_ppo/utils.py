...
import os, time, random, pathlib, yaml, collections
...
##############################################################################################
# Jeff's utility functions for RL training and evaluation
...
'''
def create_result_dir(name: str):
    """Create a results directory (under 'Results/') for this experiment."""
    base_dir = "Results"
...
def create_result_dir(experiment_name: str) -> str:
    ...
    path = Path("results") / f"{run_time}_{experiment_name}"
    path.mkdir(parents=True, exist_ok=False)
    return str(path)

class ReplayBuffer:
    """Simple replay buffer for DQN off-policy training."""
...
class MetricRecorder:
    """
    Pulls raw scalars from SB3’s internal logger buffer every `interval`
    steps and dumps them to Pandas for easy plotting.
    """
    ...
    def collect(self, locals_: Dict, globals_: Dict):
        self._counter += 1
        if self._counter % self.interval != 0:
            return
        logger = locals_["self"].logger
        row = {k: v for (k, v) in logger.name_to_value.items()}
        row["timestep"] = locals_["self"].num_timesteps
...
def plot_rewards(df: pd.DataFrame, out_png: str, smooth: int):
    plt.figure()
    plt.plot(df["timestep"], _rolling(df["rollout/ep_rew_mean"], smooth))
    plt.xlabel("Timesteps"); plt.ylabel("Episode reward (EMA)")
    plt.title("Reward curve")
    plt.tight_layout(); plt.savefig(out_png); plt.close()

def plot_losses(df: pd.DataFrame, out_png: str, smooth: int):
    keys = ["train/policy_gradient_loss",
            "train/value_loss",
            "train/entropy_loss"]
    plt.figure()
    for k in keys:
        if k in df:
            plt.plot(df["timestep"], _rolling(df[k], smooth), label=k.split("/")[-1])
    plt.legend(); plt.xlabel("Timesteps"); plt.ylabel("Loss"); plt.title("Loss terms")
    plt.tight_layout(); plt.savefig(out_png); plt.close()

def plot_kl(df: pd.DataFrame, out_png: str, smooth: int):
    if "train/approx_kl" not in df:
        return
    plt.figure()
    plt.plot(df["timestep"], _rolling(df["train/approx_kl"], smooth))
    plt.xlabel("Timesteps"); plt.ylabel("approx_kl"); plt.title("KL‑divergence")
    plt.tight_layout(); plt.savefig(out_png); plt.close()

# --------------- dataset helper -------------------------------------- #
def ensure_dataset(bag_dir: str, dataset_npz: str):
    """Create dataset_npz from ROS 2 bags if the file is missing."""
    if Path(dataset_npz).exists():
        print(f"[utils] dataset found → {dataset_npz}")
        return
    print(f"[utils] dataset not found, running bag2npz converter…")
    sys.path.append(".")
    bag2npz = importlib.import_module("bag2npz")
    bag2npz.BAG_DIR = Path(bag_dir)
    bag2npz.OUT_FILE = Path(dataset_npz)
    bag2npz.convert()
    print("[utils] conversion done.")
